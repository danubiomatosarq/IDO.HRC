...
function getListaPDSA(filtroGrupo, filtroMes) {
    try {
        ...
        var output = [];
        // --- Pré-processamento dos Dados Financeiros (Otimização) ---
        var varMap = {};
        var abaConsolidado = ss.getSheetByName("CONSOLIDADO (ALTERE AQUI)");
        if (abaConsolidado) {
            var dadosFin = abaConsolidado.getDataRange().getValues();
            for (var j = 1; j < dadosFin.length; j++) {
                var finRow = dadosFin[j];
                var m = normalizarMesPlanilha(finRow[14]);
                var c = limparTexto(finRow[1]);
                if (m && c) {
                    var pl = parseNumberBR(finRow[2]);
                    var ex = parseNumberBR(finRow[3]);
                    varMap[m + "_" + c] = ex - pl;
                }
            }
        }
        ...
        for (var i = 1; i < dados.length; i++) {
            ...
            // Lookup O(1) do Resultado
            var alvoM = normalizarMesPlanilha(row[2]);
            var alvoC = limparTexto(row[4]);
            var varValItem = varMap[alvoM + "_" + alvoC] !== undefined ? varMap[alvoM + "_" + alvoC] : null;
